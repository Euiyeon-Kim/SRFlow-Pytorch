...
class RRDBNet(nn.Module):
    def __init__(self, gc=32, scale=4, config=None):
        self.config = config
        nf = config.netG.RRDBencoder.nf
        nb = config.netG.RRDBencoder.nb
    ...
    def forward(self, x, get_steps=False):
        ...
        if self.config.netG.RRDBencoder.fea_up0:
...
if __name__ == '__main__':
    ...
    encoder = RRDBNet(scale=_config.scale, config=_config).to(device)
